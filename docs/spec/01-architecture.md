...
| --- | --- | --- |
| SWA config | `app/public/staticwebapp.config.json` (copied into `app/out` by the export) | `/` **rewritten** to `/index.html` (never redirected) — since 2026-08-01 the bare domain resolves a language client-side there (ADR-038); the `/` → `/es/` 302 that used to sit here pre-empted it at the edge, and `e2e/pages.spec.ts` guards against a redirect rule's return. The rewrite is what makes it work under `swa start` in proxy mode, where there is no directory for SWA to index. 404 rewrite to `/404.html`; 401 → 302 `/es/sign-in/`; long-cache headers for `/_next/static/*`; `platform.apiRuntime: "dotnet-isolated:9.0"`; the **`auth.identityProviders.customOpenIdConnectProviders`** block wiring `ebrostay` (Entra External ID) and `ebrostay-msa` (direct MSA) per ADR-035/036; convenience route rules for auth-gated paths (`/{es,en}/account/*` → `authenticated`; `…/admin/*` → `admin`) — **cosmetic only**, see §3.5. `/host/*` carries no such rule: since 2026-08-01 the owner working routes bounce in-app via `RequireOwner` instead, and `/host` itself is public. |
