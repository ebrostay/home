...
| ~~`/.auth/logout` may leave the Entra session live~~ — **RESOLVED 2026-07-31.** Tested against the deployed app: sign-out followed by sign-in gives a **fresh prompt**, no silent re-entry. No `end_session_endpoint` chaining needed; the `localeHome()` fix is sufficient. | n/a |
| ~~`nameClaimType` mismatch leaves `userDetails` empty~~ — **RESOLVED 2026-07-31.** A real principal returned `userDetails: "Tester"` from the `name` claim. Note it is the **display name**, not the email (which arrives as `preferred_username`). | n/a |
