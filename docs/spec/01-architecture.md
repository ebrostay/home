...
| --- | --- | --- |
| SWA config | `app/public/staticwebapp.config.json` (copied into `app/out` by the export) | `/` → `/es/` 302 redirect; 404 rewrite to `/404.html`; 401 → 302 `/es/sign-in/`; long-cache headers for `/_next/static/*`; `platform.apiRuntime: "dotnet-isolated:9.0"`; the **`auth.identityProviders.customOpenIdConnectProviders`** block wiring `ebrostay` (Entra External ID) and `ebrostay-msa` (direct MSA) per ADR-035/036; convenience route rules for auth-gated paths (`/{es,en}/account/*`, `…/host/*` → `authenticated`; `…/admin/*` → `admin`) — **cosmetic only**, see §3.5. |
